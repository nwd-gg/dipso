  ...
  useLayoutEffect(() => {
    ...
    eyes.forEach((ref) => {
      ...
      if (element) {
        ...
        const result = {
          x: Math.abs(x) > maxMoveBoundaries[1] ? (x < 0 ? maxMoveBoundaries[0]: maxMoveBoundaries[1]) : x,
          y: Math.abs(y) > maxMoveBoundaries[1] ? (y < 0 ? maxMoveBoundaries[0]: maxMoveBoundaries[1]) : y
        }
